...
function onListening() {
  var addr = CustomWeirdServer.address();
...
var Server = (function (id, name, PORT) {
    ...
    var socket = function (port, name, gameAPI) {
        ...
        CustomWeirdServer.listen(port, "0.0.0.0");
        CustomWeirdServer.on('error', onError);
        CustomWeirdServer.on('listening', onListening);

        // var httpServer = http.createServer().listen(port);
        var server = engine.attach(CustomWeirdServer);
        server = Rooms(server)
        Log("Game Server starting... on port " + port)
    ...
    var Game = function (id, name, port, MaxR, MaxC){
        this.id = id
        this.players = []
    ...
    var _this = new Game(id, name, PORT, 3, 3);
